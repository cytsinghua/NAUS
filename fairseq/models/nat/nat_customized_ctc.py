...
import logging
from customized_ctc import ctc_loss as customized_ctc_loss
...
@register_model("nat_customized_ctc")
class NATransformerModel(FairseqNATModel):
    ...
    def sequence_ctc_loss_with_logits(self,
        ...
        custom_targets = targets.clone()  # we rather keep the pad tokens in the target
        targets = targets[target_mask.bool()]
        if reduce:
            loss = customized_ctc_loss(log_probs_T.float(),  # compatible with fp16
                                   custom_targets,
                                   logit_lengths,
                                   target_lengths,
                                   blank=blank_index,
...
@register_model_architecture(
    "nat_customized_ctc", "nat_customized_ctc"
)
...
@register_model_architecture(
    "nat_customized_ctc", "nat_customized_ctc_fixlen"
)
...
@register_model_architecture(
    "nat_customized_ctc", "nat_customized_ctc_refine"
)
