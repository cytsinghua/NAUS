...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        # initialize
        # if model.use_bert_weights:
        #     pseudo_src_tokens = torch.tensor([0,  101, 1026, 1064, 4895, 2243, 1064, 1028,  102],device='cuda').unsqueeze(0)
        #     pseudo_lengths = torch.tensor(9, device="cuda")
        #     encoder_out = model.forward_encoder([pseudo_src_tokens, pseudo_lengths])
        ...
        for step in range(self.max_iter + 1):
            ...
            if model.use_bert_weights:
                src_tokens[src_tokens == 1] = model.bert_tokenizer.pad_token_id
                bert_input_sample = src_tokens
                prev_output_tokens = model.initialize_output_tokens_by_src_tokens(bert_input_sample)
                # pass
                # src_lengths -= 1
                embeded_tokens = model.bert_model.base_model.embeddings(bert_input_sample)
                last_hidden_state = model.bert_model.base_model.encoder(embeded_tokens).last_hidden_state
                embedding_weights = model.bert_model.embeddings.word_embeddings.weight
                encoder_out = torch.matmul(last_hidden_state, embedding_weights.transpose(0, 1))

            decoder_out = model.forward_decoder(src_tokens,
