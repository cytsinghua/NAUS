    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                summary_writer = getattr(self.model, 'model_tb', None)
                if summary_writer is not None:
                    summary_writer.add_histogram("prob_predictor", self.model.map_prob_predictor.weight, self._num_updates)
                # if "latent_dist" in logging_output:
                #     summary_writer.add_histogram("latent_dist", logging_output["latent_dist"], self._num_updates)
