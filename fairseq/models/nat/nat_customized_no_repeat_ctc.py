...
from customized_no_repeat_ctc import ctc_loss as customized_no_repeat_ctc_loss
...
@register_model("nat_customized_no_repeat_ctc")
class NATransformerModel(FairseqNATModel):
    def __init__(self, args, encoder, decoder):
        super().__init__(args, encoder, decoder)
        # Nat_ctc should not be able to use the bert weights or tokens
        self.use_bert_weights = False
        self.use_bert_tokens = args.use_bert_tokens
    ...
    def sequence_ctc_loss_with_logits(self,
        ...
        if reduce:
            loss = customized_no_repeat_ctc_loss(log_probs_T.float(),  # compatible with fp16
                custom_targets,
                logit_lengths,
                target_lengths,
                blank=blank_index,
            ...
            loss = torch.clip(loss, min=0, max=100)
            loss = loss.mean()
        else:
            loss = customized_no_repeat_ctc_loss(
    ...
    def ctc_length_control_initialization(self, logits, desired_length, device):
        """
    ...
    def column_inference(self, column, current_log_prob, desired_length, ctc_blank_token_id, replacing_value,
                         dp_log_prob_table, dp_sequence_table):
        """
        Perform table (prob table and prefix table) filling for a single column
        """
        most_probable_values, most_probable_indices = current_log_prob.topk(2)
        if most_probable_indices[0] == ctc_blank_token_id:
            # If the probability of the blank token is the highest
            most_probable_non_blank_token = most_probable_indices[1]
            most_probable_non_blank_prob = most_probable_values[1]
        else:
            # the probability of the blank token is not the highest
            # the probability of some other tokens is the highest
            most_probable_non_blank_token = most_probable_indices[0]
            most_probable_non_blank_prob = most_probable_values[0]
        blank_prob = current_log_prob[ctc_blank_token_id]

        # For first row, we can only assign blank token
        dp_log_prob_table[0, column] = blank_prob
        if column != 0:
        ...
        if column + 1 < desired_length:
            dp_log_prob_table[column + 1, column] = most_probable_non_blank_prob  # Assign the probability of the current word to the prob matrix
            if column != 0:
                ...
                dp_sequence_table[column + 1, column, 0:column] = dp_sequence_table[column, column - 1, 0:column].clone()
            dp_sequence_table[column + 1, column, column] = most_probable_non_blank_token.item()
        middle_rows_num = min(column+1 - 1, desired_length-1)
        ...
        if middle_rows_num > 0:
            ...
            middle_rows_prob_matrix[:, 0] += most_probable_non_blank_prob  # add the new word prob to matrix
            middle_rows_prob_matrix[:, 1] += blank_prob
            ...
            non_blank_index = (most_probable_prev_rows == 0).nonzero(as_tuple=True)[0]+1
            blank_index = (most_probable_prev_rows == 1).nonzero(as_tuple=True)[0]+1
            padding_index = torch.arange(0, middle_rows_num, device=middle_rows_prob_matrix.device)
            # recover the index by adding some padding
            most_probable_prev_rows = most_probable_prev_rows + padding_index
            selected_token_prev_rows = dp_sequence_table[:, column-1, 0:column][most_probable_prev_rows]
            # record the probability to the table
            dp_log_prob_table[1:end_row, column] = selected_prob_prev_rows.clone()
            # record the chosen sequence to the table
            dp_sequence_table[1:end_row, column, 0:column] = selected_token_prev_rows.clone()

            assert most_probable_non_blank_token.item() >=0
            if len(non_blank_index) > 0:
                dp_sequence_table[non_blank_index, column, column] = most_probable_non_blank_token.item()
            if len(blank_index) > 0:
                dp_sequence_table[blank_index, column, column] = ctc_blank_token_id


    def my_remove_adjacent(self,nums):
            return [a for a, b in zip(nums, nums[1:] + [not nums[-1]]) if a != b]

    def ctc_length_control(self, logits, length_control, desired_length, ctc_pad_token_id=1, ctc_bos_token_id=0,
        ...
        There are five different cases:
            ...
            4) If the current table slot is the (i+1)-th row & i-th column, we must continue from the i-th row and the
                (i-1)-th column and generate a new token as the token in the i-th row and the (i-1)-th column.
            5) If the current table slot is i-th row & j-th column, where i < j, there are two choices to pick.
                The first one is to continue with the subsequence ended at table slot (i-1, j-1) and generate a
                non-blank token. The second choice is to continue with the subsequence ended at table slot (i, j-1) and
                generate either a blank token token.
        """
        ############################################# Initialization #############################################
        ...
        prob_sequence, ctc_sequence_length, dp_log_prob_table, dp_sequence_table \
            = self.ctc_length_control_initialization(logits, desired_length, device)
        ...
        for column in range(0, ctc_sequence_length):
            ...
            self.column_inference(column, current_log_prob, desired_length, ctc_blank_token_id,
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs):
        ...
        if self.length_control == "greedy_dp" or self.length_control == "greedy_dp_scope_search":
            ...
            for i in range(0, len(output_logits)):
                current_sentence = self.ctc_length_control(output_logits[i], self.length_control, self.desired_length,
...
@register_model_architecture(
    "nat_customized_no_repeat_ctc", "nat_customized_no_repeat_ctc"
)
...
@register_model_architecture(
    "nat_customized_no_repeat_ctc", "nat_customized_no_repeat_ctc_fixlen"
)
...
@register_model_architecture(
    "nat_customized_no_repeat_ctc", "nat_customized_no_repeat_ctc_refine"
)
